...
class FLAConfig:
    bin_tolerance: int = 2               
    min_peak_height: int = 50            # Minimum valid peak intensity
    min_peak_position: int = 50          # Ignore peaks before this position (likely noise)
    relative_peak_threshold: float = 0.1 # Fraction of max intensity to consider a peak valid within a bin
    ...
    stutter_ratio_threshold: float = 0.15  # Default max allowed intensity ratio for stutter
    stutter_tolerance: float = 1.0

    global_het_ratio_lower_bound: float = 0.35

    # New: Instrument saturation logic
    instrument_saturation_value: float = 30000.0  # Example saturation level
    gaussian_fit_window: int = 5                 # Number of data points on each side of the peak to use for local fitting
