...
with st.expander("Advanced Options"):
    ...
    min_peak_height = st.number_input("Minimum Peak Height", min_value=10, max_value=1000, value=1000)
    ...
    intensity_weight = st.number_input("Intensity Weight", min_value=0.1, max_value=5.0, value=3.0)
...
for i, marker in enumerate(st.session_state.marker_list):
    ...
    with marker_cols[3]:
        ...
        with st.expander("Options", expanded=False):
            ...
            marker["stutter_ratio_threshold"] = st.number_input("Stutter Ratio Threshold", min_value=0.01, max_value=1.0, value=marker["stutter_ratio_threshold"], key=f"stutter_ratio_{i}")
            marker["stutter_tolerance"] = st.number_input("Stutter Tolerance", min_value=0.1, max_value=5.0, value=marker["stutter_tolerance"], key=f"stutter_tolerance_{i}")
            marker["stutter_direction"] = st.segmented_control("Stutter Direction", selection_mode="single", options=["left", "both", "right"], default=marker["stutter_direction"], key=f"stutter_direction_{i}")
