...
def edit_marker_overrides_dialog(marker_index: int):
    ...
    with col_delete:
        if st.button(
            ...
            type="primary",
            key=f"delete_marker_{marker_index}",
        ):
...
def markers_upload_dialog():
    ...
    if marker_file:
        try:
            ...
            st.session_state.marker_list = [
                MarkerConfig(**entry) for entry in marker_data
            ]
...
def marker_config_ui():
    ...
    with col_filename:
        marker_config_filename = st.text_input(
            "Filename", value="marker_config.json", label_visibility="collapsed"
        )
    ...
    with col_download:
        st.download_button(
            ...
            icon=(
                ...
                if bool(st.session_state.marker_list)
                else ":material/file_download_off:"
            ),
            disabled=not bool(st.session_state.marker_list),
        )

    with st.container(
        height=300 if st.session_state.marker_list else "content",
        border=False,
    ):
        for i, marker in enumerate(st.session_state.marker_list):
            cols = st.columns([2, 2, 1, 1, 1, 2], vertical_alignment="bottom")

            marker.marker = cols[0].text_input(
                "Marker", value=marker.marker, key=f"marker_{i}"
            )
            marker.channel = cols[1].selectbox(
                "Channel",
                ["6-FAM", "VIC", "NED", "PET"],
                index=["6-FAM", "VIC", "NED", "PET"].index(marker.channel),
                key=f"channel_{i}",
            )
            marker.repeat_unit = cols[2].number_input(
                "Repeat", 1, 10, value=marker.repeat_unit or 1, key=f"repeat_{i}"
            )
            min_bin = cols[3].number_input(
                "Min", 30, 500, value=marker.bins[0], key=f"bmin_{i}"
            )
            max_bin = cols[4].number_input(
                "Max", 30, 500, value=marker.bins[1], key=f"bmax_{i}"
            )
    ...
    if st.session_state.marker_list:
        ...
        if st.button(
            "Add Marker", icon=":material/add_circle_outline:", use_container_width=True
        ):
            st.session_state.marker_list.append(
                MarkerConfig(
                    marker="Marker1",
