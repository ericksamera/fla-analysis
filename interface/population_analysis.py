...
def export_section(samples, genotype_df):
    ...
    st.download_button(
        "Download Export", data=csv_data, file_name=exporter.filename(), mime="text/csv"
    )
...
def run():
    ...
    genotype_matrix = {
        sid: s.marker_results for sid, s in samples.items() if s.marker_results
    }
    ...
    genotype_df["_Pop"] = genotype_df["sample_uid"].map(
        lambda uid: (
            samples[uid].metadata.get("Population", "Unknown")
            if uid in samples
            else "Unknown"
        )
    )
    ...
    if st.button("Recalculate Distance Matrix"):
        ...
        calc = DistanceCalculator(
            genotype_matrix, marker_configs, sample_metadata=metadata
        )
    ...
    meta_keys = sorted(
        ...
        | {k for s in samples.values() for k in s.metadata.keys()}
    )
    color_by = st.selectbox(
        "Color PCoA by metadata field", options=["None"] + meta_keys, index=0
    )
    ...
    if debug_mode:
        with st.expander("📜 Debug Log"):
            st.text_area(
                "Distance Log",
                st.session_state.get("distance_debug_log", ""),
                height=300,
            )
